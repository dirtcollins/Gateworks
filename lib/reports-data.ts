...
// Aggregates revenue, AOV, payment status, AR aging, and gross margin from
// Supabase orders. Shared by the admin reports page and the Design Lab.
export async function fetchReportData(): Promise<ReportData> {
  const admin = getSupabaseAdminClient();
  if (!admin) {
    return emptyReportData(false);
  }
